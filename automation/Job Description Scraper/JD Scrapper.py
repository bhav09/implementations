...
driver.get('https://www.linkedin.com/login?session_redirect=https%3A%2F%2Fwww%2Elinkedin%2Ecom%2Fin%2Fbhavishya-pandit%2F&fromSignIn=true&trk=public_authwall_profile-login-link')
...
signin_button = driver.find_element_by_xpath('//button[@class="btn__primary--large from__button--floating"]')
signin_button.click()
...
while True:
    ...
    if job_url in current_url:
        try:
            if driver.find_element_by_xpath('//span[@class="artdeco-inline-feedback__message"]'): #shows that you have applied
                count += 1
                if count == 1:
                    job_title = driver.find_element_by_class_name('jobs-details-top-card__job-title').text #job title
                    company_name = driver.find_element_by_class_name('jobs-details-top-card__company-info') #Name of the comapny
                    job_desc = driver.find_element_by_class_name('jobs-description-content__text').text #job description
